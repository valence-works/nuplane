...
/// </summary>
internal sealed class NuGetVersionRangeEvaluator : IVersionRangeEvaluator
{
    /// <inheritdoc />
    public VersionResolutionResult SelectBestMatch(string versionRange, IReadOnlyList<string> availableVersions)
    {
        ...
        {
            return new VersionResolutionResult(false, null, 0, "Resolution failed: no versions available.");
        }
        ...
        if (string.IsNullOrWhiteSpace(versionRange))
        {
            ...
            if (stable.Count == 0)
            {
                return new VersionResolutionResult(false, null, candidateCount, "Resolution failed: no stable versions available.");
            }

            var max = stable.Max()!;
            return new VersionResolutionResult(true, max.ToNormalizedString(), candidateCount, null);
        }
        ...
        {
            return new VersionResolutionResult(false, null, candidateCount, $"Resolution failed: invalid version range '{versionRange}'.");
        }
        ...
        {
            return new VersionResolutionResult(false, null, candidateCount, $"Resolution failed: no version matched range '{versionRange}'.");
        }

        var best = satisfying.Max()!;
        return new VersionResolutionResult(true, best.ToNormalizedString(), candidateCount, null);
    }
