...
public sealed class OperationalSnapshotProjector
{
    ...
    private List<string> BuildDegradedReasons()
    {
        ...
        if (_healthEvaluator.LastUnloadPendingCount > 0)
            reasons.Add($"unload-pending:{_healthEvaluator.LastUnloadPendingCount}");
        ...
        if (_healthEvaluator.LastLoaderFailureCount > 0)
            reasons.Add($"loader-failures:{_healthEvaluator.LastLoaderFailureCount}");
