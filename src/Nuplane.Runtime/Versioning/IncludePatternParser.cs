...
internal static class IncludePatternParser
{
    ...
    public static ParsedIncludePattern Parse(string pattern)
    {
        ...
        {
            return new ParsedIncludePattern(string.Empty, string.Empty, pattern ?? string.Empty);
        }
        ...
        if (bracketIndex > 0)
        {
            ...
            return new ParsedIncludePattern(glob, versionRange, pattern);
        }
        ...
        if (lastSpaceIndex < 0)
        {
            return new ParsedIncludePattern(trimmed, string.Empty, pattern);
        }
        ...
        if (suffix.Length > 0 && char.IsAsciiDigit(suffix[0]))
        {
            ...
            return new ParsedIncludePattern(glob, suffix, pattern);
        }
