...
public sealed class SharedAssemblyPolicyContractTests
{
    ...
    public void SharedPolicy_UsesNameTokenMajorMatching()
    {
        ...
        Assert.True(matcher.IsMatch(asm, [new SharedAssemblyPolicyEntry(asm.Name!, token, asm.Version!.Major)]));
        Assert.False(matcher.IsMatch(asm, [new SharedAssemblyPolicyEntry(asm.Name!, token, asm.Version!.Major + 1)]));
    }
}
