...
- Use `NuplaneHostIntegratedLoader.LoadActivePackagesAsync(stateFilePath, options, ct)` when a short-lived worker process must load the host's active package set into assemblies exactly the way that host's `HostIntegrated` loading does — same graph grouping, same load contexts, same asset selection, same `Default.Resolving` hook — without a running host. Overloads accept `StoreRegistryOptions` or an already-read `IReadOnlyList<ActivePackage>`. The load is irreversible for the process; see [Usage Guide: Host-free loading of the active package set](docs/wiki/Usage-Guide.md#host-free-loading-of-the-active-package-set).
