...
- Use `NuplaneHostIntegratedLoader.LoadActivePackagesAsync(activePackages, options, ct)` when a short-lived worker process must load that active package set into assemblies exactly the way a host's `HostIntegrated` loading does — same load contexts, same asset selection, same `Default.Resolving` hook — without a running host. The load is irreversible for the process; see [Usage Guide: Host-free loading of the active package set](docs/wiki/Usage-Guide.md#host-free-loading-of-the-active-package-set).
