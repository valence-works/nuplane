...
- **FR-006B**: Nuplane MUST expose a loading-owned convenience assembly-catalog service that serves as the default host-facing loading integration API, supports all-packages, active-package-by-id, and exact package/version reads over the current active loaded package set, applies sane defaults for common host integrations, and returns an empty result or no match when loading is disabled, stale, inactive, or not loaded so callers can opt into `ILoadingCatalog` only when they need detailed availability reasoning.
...
- **FR-008B**: When Nuplane provides convenience type scanning helpers, those helpers MUST be positioned as optional layers over the assembly-catalog model and SHOULD support active-package semantics that match the assembly catalog when version-specific inputs are not supplied.
