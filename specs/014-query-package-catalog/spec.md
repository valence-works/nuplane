...
- **SC-006**: In sample validation, a repository sample host can map both the core admin routes and the loading-owned route, query Nuplane for active assembly scan candidates and explicit plugin discovery output, and use that result to discover at least one expected host-relevant type from a loaded package without depending exclusively on observer callbacks.
