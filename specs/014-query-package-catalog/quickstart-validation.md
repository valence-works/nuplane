...
| 5 | Loading catalog respects active-package boundaries and hides discovered-type identities | PASS | `LoadingCatalogBoundaryTests` verifies only active packages are projected, scan candidates stay under the active install path, and public loading contracts expose assembly metadata rather than discovered types. |
...
dotnet test test/Nuplane.Loading.Tests/Nuplane.Loading.Tests.csproj --filter "FullyQualifiedName~LoadingCatalogTests|FullyQualifiedName~LoadingCatalogBoundaryTests|FullyQualifiedName~LoadingCatalogObservabilityTests|FullyQualifiedName~PackageLoaderCatalogCandidateTests|FullyQualifiedName~LoadingCatalogHealthTests|FullyQualifiedName~LoadingOwnershipContractTests"
dotnet test test/Nuplane.Integration.Tests/Nuplane.Integration.Tests.csproj --filter "FullyQualifiedName~ActivePackageCatalogConsistencyIntegrationTests|FullyQualifiedName~ActivePackageCatalogRestartIntegrationTests|FullyQualifiedName~ObserverQueryFirstPackageCatalogContractTests|FullyQualifiedName~LoadingCatalogEndpointIntegrationTests|FullyQualifiedName~LoadingCatalogIntegrationTests|FullyQualifiedName~AdminReadEndpointContractTests|FullyQualifiedName~AdminEndpointOwnershipContractTests|FullyQualifiedName~OperationalStateContributorIntegrationTests"
dotnet pack samples/Nuplane.Sample.Plugin/Nuplane.Sample.Plugin.csproj -c Debug
dotnet build samples/Nuplane.Sample.AspNetCore/Nuplane.Sample.AspNetCore.csproj -c Debug
...
| Nuplane.Loading.Tests | loading catalog + boundaries + observability + scan candidates + loading health + ownership | 16 | 0 |
| Nuplane.Integration.Tests | active catalog consistency/restart/query-first + loading route/restart + admin route ownership/state contributor | 13 | 0 |
...
```text
Test summary: total: 551, failed: 0, succeeded: 551, skipped: 0
Build succeeded in 54,3s
```
