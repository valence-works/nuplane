...
## Goal
Validate that Nuplane exposes a query-first active package catalog from core runtime services, a separate loading catalog from the optional loading module, and an operational-state surface that remains distinct from package inventory.
...
## 4) Validate loading availability states
1. Run the host without the loading module installed and query the admin/operator loading read.
2. Confirm loading is reported as unavailable rather than as an empty successful catalog.
...
4. Restart a loading-enabled host before any current-process refresh and confirm the loading catalog reports `Stale` while the active package catalog remains readable.
...
3. Query the loading catalog for active assembly scan candidates.
...
5. Confirm the sample flow does not rely solely on observer callbacks to know which package assemblies to inspect.
...
- Integration/admin tests proving loading-unavailable behavior when the module is absent.
...
- Loading reads clearly distinguish `Unavailable`, `Disabled`, `Stale`, `Loaded`, and `Failed` scenarios at the correct layer.
