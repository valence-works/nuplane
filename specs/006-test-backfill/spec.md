...
- Q: When one `IDesiredPackageSource` throws in `DesiredStateAggregator`, what is the observable contract (suppress silently, log only, or surface in result)? → A: Catch per-source; aggregate healthy results; surface the error in a `SourceErrors` collection on the returned result — not silent, not log-only.
- Q: How should the FR-014 concurrent-access test for `DesiredSourceSnapshotCache` be structured (deterministic `TaskCompletionSource`-gated fake, or skip in favour of sequential call test)? → A: Use a `TaskCompletionSource`-gated fake `IDesiredPackageSource`; launch two concurrent tasks; verify exactly one source read occurred after both tasks complete.
...
1. **Given** `DesiredStateAggregator` is constructed with two mocked `IDesiredPackageSource` instances, **When** one source returns packages and one throws, **Then** the aggregated result contains the healthy source's packages and the faulting source's exception is recorded in `SourceErrors` on the result.
...
- **FR-010**: `Reconciliation/DesiredStateAggregatorTests.cs` MUST cover: single-source aggregation; multi-source merge with all sources healthy; one source throws — healthy sources' results MUST still be aggregated and the exception MUST appear in `SourceErrors` on the returned result; zero sources configured (empty result, empty `SourceErrors`, no exception).
- **FR-011**: `Reconciliation/AllowlistGateTests.cs` MUST cover: all packages permitted; one package blocked by allowlist; all packages blocked (empty output set); allowlist empty (all packages permitted by default).
...
- **FR-013**: `Packages/PackageCleanupServiceTests.cs` MUST cover: no cleanup needed (policy satisfied, zero removals); two versions eligible for removal (both scheduled); cleanup error for one version does not abort removal of others; cancellation token honoured — passing an already-cancelled `CancellationToken` MUST cause `OperationCanceledException` to be thrown before any removal is executed.
- **FR-014**: `Sources/DesiredSourceSnapshotCacheTests.cs` MUST cover: cache hit returns same snapshot reference; cache miss triggers re-read; expired cache entry triggers re-read; concurrent access — a `TaskCompletionSource`-gated fake source MUST be used to launch two simultaneous readers; after both complete exactly one source read MUST have occurred (the second caller reuses the in-flight result).

**Group 3 — Nuplane.Loading Unit Tests (FR-015–FR-019)**

- **FR-015**: A new test project `test/Nuplane.Loading.Tests/Nuplane.Loading.Tests.csproj` MUST be created and added to `Nuplane.sln`. The project MUST reference `src/Nuplane.Loading/` and `src/Nuplane.Loading.Abstractions/` and MUST use xUnit and NSubstitute (consistent with the existing test projects). A companion fixture project `test/Nuplane.Loading.Tests.Fixtures/Nuplane.Loading.Tests.Fixtures.csproj` MUST also be created and added to `Nuplane.sln`; it produces a minimal class library assembly used as the load target in ALC tests.
...
- The existing test projects use xUnit, NSubstitute, and `Microsoft.Extensions.Logging.Abstractions` for mocking `ILogger<T>`. `Nuplane.Loading.Tests` adopts the same stack.
