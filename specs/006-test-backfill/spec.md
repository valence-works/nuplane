...
- **FR-015**: A new test project `test/Nuplane.Loading.Tests/Nuplane.Loading.Tests.csproj` MUST be created and added to `Nuplane.sln`. The project MUST reference `src/Nuplane.Loading/` and `src/Nuplane.Loading.Abstractions/` and MUST use xUnit and NSubstitute (consistent with all test projects; see Decision 1). A companion fixture project `test/Nuplane.Loading.Tests.Fixtures/Nuplane.Loading.Tests.Fixtures.csproj` MUST also be created and added to `Nuplane.sln`; it produces a minimal class library assembly used as the load target in ALC tests.
